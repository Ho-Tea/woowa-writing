...
- 필자는 `Spring Boot`를 활용하여 **효과적인 로깅의 3가지 공통점**을 적용해 보았습니다. 
...
1. **Http Status**
- 설명: HTTP 상태 코드는 클라이언트의 요청에 대한 서버의 응답 상태를 나타냅니다. 이를 로깅하면 요청이 성공적으로 처리되었는지(2xx), 클라이언트 오류(4xx)나 서버 오류(5xx)가 발생했는지 파악할 수 있습니다.
- 활용: 오류 상태 코드(예: 400, 500)가 반복적으로 발생하는 패턴을 파악해 문제를 진단할 수 있으며, 특정 API에서 주기적으로 실패하는 경우 이를 추적할 수 있습니다.
2. **Http Method**
- 설명: HTTP 메서드(GET, POST, PUT, DELETE 등)는 요청이 어떤 동작을 수행하는지 명확히 알려줍니다. 어떤 메서드가 문제가 있는지 로그를 통해 파악할 수 있습니다.
- 활용: 특정 HTTP 메서드에서 예기치 않은 동작이 발생할 경우 이를 추적할 수 있습니다. 예를 들어, POST 요청에서만 발생하는 오류가 있을 때 그 요청을 쉽게 구분해 진단할 수 있습니다.
3. **Request URI**
- 설명: 요청 URI는 사용자가 요청한 자원 경로를 나타냅니다. URI를 로깅하면 어떤 엔드포인트에서 문제가 발생했는지 쉽게 파악할 수 있습니다.
- 활용: 특정 URI에서 반복적으로 에러가 발생하거나 성능 이슈가 생길 때 이를 통해 원인을 분석할 수 있습니다. 또한, 예상치 못한 URI에 대한 요청이 자주 발생할 경우, 잘못된 API 호출을 파악하는 데 유용합니다.
4. **Token 유무**
- 설명: 보통 인증 및 권한 부여와 관련된 토큰(JWT 등)의 유무를 기록합니다. 인증 관련 오류나 권한 문제를 추적할 수 있습니다.
- 활용: 요청 시 인증 토큰이 없거나 유효하지 않을 때, 이를 기록하여 인증 문제를 쉽게 파악할 수 있습니다. 특히, 보안 문제를 해결하는 데 큰 도움이 됩니다.
5. **Processing Time (소요 시간)**
- 설명: 요청을 처리하는 데 걸린 시간을 기록합니다. 이를 통해 특정 요청의 성능 이슈를 확인할 수 있습니다.
- 활용: 응답 시간이 지나치게 길어진 요청을 파악하여, 성능 병목 현상을 찾고 개선할 수 있습니다. 예를 들어, 응답 시간이 긴 특정 엔드포인트를 분석하여 최적화할 수 있는 단서를 제공합니다.
...
**Logback의 주요 특징**
- **높은 성능** 
  - `Logback`은 `Log4j`보다 빠른 로깅 성능을 제공합니다. 특히, 대규모 애플리케이션에서 많은 양의 로그를 처리할 때 성능 차이가 두드러집니다.
...
- **계층적 로깅 (Logger Hierarchy)**
  - `Logback`은 로깅의 계층 구조를 기반으로 하여, 특정 패키지나 클래스에 대한 로깅을 세밀하게 제어할 수 있습니다. 패키지나 클래스 단위로 로그 레벨을 다르게 설정할 수 있습니다.
예를 들어, 특정 패키지는 `DEBUG` 레벨, 다른 패키지는 `INFO` 레벨로 설정할 수 있습니다.

- **로그 포맷 관리**
  - `Logback`은 로그 메시지를 포맷팅할 수 있는 다양한 옵션을 제공하며, 타임스탬프, 스레드 이름, 로그 레벨, 클래스 이름 등 다양한 정보를 로그에 포함시킬 수 있습니다. 커스텀 로그 포맷을 정의하여 로그를 보다 직관적이고 분석 가능하게 만들 수 있습니다.

- **다양한 출력 대상**
  - `Logback`은 로그를 콘솔, 파일, 네트워크 소켓, 데이터베이스 등 다양한 대상에 기록할 수 있습니다. 여러 출력 대상을 동시에 지정할 수 있어, 로그를 다수의 장소에 저장하는 것도 가능합니다.

- **조건부 로깅**
  - `Logback`은 특정 조건이 충족될 때만 로그를 기록하는 기능을 제공합니다. 이를 통해 불필요한 로그 기록을 방지하고, 로그의 양을 효과적으로 제어할 수 있습니다.

**Logback 구성 요소**
`Logback`은 주로 `Logger`, `Appender`, `Layout` 세 가지 주요 구성 요소로 이루어져 있습니다.

- `Logger`
  - 로깅 메시지를 생성하는 객체입니다. 
  - 개발자는 `Logger` 객체를 사용하여 로그를 기록할 수 있습니다.
  - `Logger`는 패키지나 클래스 단위로 생성되며, 계층적으로 관리됩니다.
  - 각 `Logger`는 로그 레벨을 설정하여, 특정 수준 이상의 메시지만 기록할 수 있습니다.

- `Appender`
  - `Logger`에서 생성된 로그 메시지를 어디에 출력할지를 결정하는 역할을 합니다. 로그를 콘솔에 출력할지, 파일에 기록할지, 또는 원격 서버로 보낼지 등을 정의합니다.
  - 자주 사용되는 `Appender` 종류
    - `ConsoleAppender`: 로그를 콘솔에 출력합니다.
    - `FileAppender`: 로그를 파일에 저장합니다.
    - `RollingFileAppender`: 로그 파일이 일정 크기 이상 커지면 새로운 파일을 생성하여 로그를 분할 관리합니다.
    - `SocketAppender`: 로그를 네트워크를 통해 원격 서버로 전송합니다.

- `Layout`
  - 로그 메시지의 형식을 정의하는 역할을 합니다. `Layout`을 통해 로그 출력 형식을 설정하여 로그가 일정한 포맷을 따르도록 할 수 있습니다.
  - 자주 사용되는 `Layout`
    - `PatternLayout`: 사용자 정의 패턴으로 로그 메시지를 포맷팅할 수 있습니다.
    - `HTMLLayout`: 로그 메시지를 HTML 형식으로 포맷팅할 수 있습니다.
...
### **Prod**
- 비즈니스 로직 상 의도한 에러(ex: `StaccatoException - 커스텀 예외`)인 경우 → `INFO Level`
- 예상하지 못한 에러면서 **비정상적인 사용자 행동**이나 **잘못된 입력**으로 인해 발생한 예외의 경우(`HttpMessageNotReadableException`, `ForbiddenException`) → `WARN Level`
<img src="prodError.png">

- `RunTimeException` → `ERROR Level`
- `DB SQL` 관련 `Exception` 중 크리티컬한(`Error`) 것들만 `GlobalExeptionHandelr`에 등록
  - `CannotCreateTransactionException`
    - 이 예외는 트랜잭션을 생성할 수 없을 때 발생합니다. 트랜잭션을 시작하는 단계에서 문제가 발생했을 경우 주로 던져지는 예외입니다. 일반적으로 데이터베이스 연결 문제나 트랜잭션 관리자의 오류 등으로 인해 트랜잭션을 열 수 없을 때 발생합니다.
  - `DataAccessException`
    - `DataAccessException은` 데이터 접근 작업 전반에서 발생하는 예외의 기본 클래스입니다. 구체적으로 SQL 쿼리 실행 중 발생한 오류, 트랜잭션 오류, 데이터 무결성 위반 등 다양한 데이터베이스 관련 예외를 포괄하는 범용적인 예외입니다. 
  - `TransactionSystemException`
    - `TransactionSystemException`은 트랜잭션 관리 시스템 자체에서 발생하는 내부 오류를 나타냅니다. 이 예외는 일반적으로 트랜잭션의 커밋, 롤백 또는 시작 과정에서 시스템 내부적으로 발생한 예외를 포장하여 던집니다. 트랜잭션과 관련된 시스템 오류나 트랜잭션 매니저 내부에서 처리되지 않은 예외로 인해 발생할 수 있습니다.
...
- `RuntimeException`은 주로 **서버 측의 문제**로 인해 발생하는 예외입니다. 이 예외가 발생하면 서버는 예상치 못한 상태가 되어 **정상적인 요청 처리**가 불가능해지므로 **심각한 문제**로 간주해야 합니다.
- 이 예외는 애플리케이션에서 **즉각적인 조치**가 필요할 수 있는 문제를 의미하므로, 이를 **`error`** 로그로 기록하는 것이 적절합니다.
- **서버 오류(500 Internal Server Error)** 는 애플리케이션이 제대로 동작하지 않는다는 신호이기 때문에, 이러한 상황은 반드시 개발자나 운영자가 빠르게 인지해야 하는 **중요한 오류**입니다.
...
- **`info`** 로그를 사용해야 하는 경우:
  ...
  - `StaccatoException`이 자주 발생할 수 있는 상황이면서도, 처리 후 시스템이 정상적으로 동작하고 경고할 필요가 없으면 **`info`** 로그로 처리하는 것이 맞습니다.
- **`warn`** 로그를 사용해야 하는 경우:
  - 예외가 예상된 것이지만, **비정상적인 사용자 행동**이나 **잘못된 입력**으로 인해 발생한 예외라면, 시스템에 큰 문제가 없더라도 **경고**를 남겨야 합니다.
  - 예를 들어, 사용자가 규칙을 위반하거나, 시스템의 정상적이지 않은 흐름이 반복적으로 발생할 수 있다면 **`warn`** 로그가 적합합니다.
